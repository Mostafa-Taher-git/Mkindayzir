...
async def get_migration_status(user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "database_url": settings.DATABASE_URL if settings.DATABASE_PROVIDER == "postgresql" else str(db_path),
...
async def rollback_migration(user: dict = Depends(get_current_user)):
    """Rollback to SQLite."""
    result = await migration_service.rollback(__import__("uuid").uuid4().hex)
    return result
