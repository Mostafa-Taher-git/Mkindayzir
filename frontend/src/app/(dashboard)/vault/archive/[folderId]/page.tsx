...
export default function VaultArchiveFolderPage() {
  ...
  const folder = archiveFolders.find((f) => f.id === folderId);

  const { data: itemsData, isLoading } = useArchiveItems({
    folderId: folderId,
    ...
    return (
      <div className="flex h-full">
        ...
        <div className="flex-1 flex items-center justify-center">
          <p className="text-sm text-muted-foreground">Archive folder not found.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Default archive folder · {folder.entityType ?? "type"}</p>
          </div>
          <Button asChild variant="outline">
            <Link to="/vault/archive">All archived</Link>
          </Button>
        </div>
        ...
        <div className="space-y-4">
          ...
          {isLoading ? (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-40 bg-muted rounded-lg animate-pulse" />)}
            </div>
          ) : items.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <p className="text-lg font-semibold mb-1">Nothing in {folder.name} yet</p>
              <p className="text-sm text-muted-foreground">Items archived from this type will land here.</p>
            </div>
          ) : viewMode === "grid" ? (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {items.map((item) => (
                <div key={item.id} className="rounded-lg border bg-card p-4 hover:shadow-md transition-all cursor-pointer" onClick={() => setOpenItemId(item.id)}>
                  ...
                  {item.summary && <p className="text-xs text-muted-foreground line-clamp-2 mb-3">{item.summary}</p>}
                  ...
                  {items.map((item) => (
                    <tr key={item.id} className="border-b hover:bg-muted/50 transition-colors">
                      <td className="px-4 py-3">
                        <button onClick={() => setOpenItemId(item.id)} className="text-sm font-medium hover:text-primary transition-colors">
                          {item.title}
                        </button>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        ...
                        <Button size="sm" variant="ghost" onClick={() => onPermDelete(item)}>Delete</Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {openItem && (
        <ArchiveDrawer item={openItem} archiveFolders={archiveFolders} onClose={() => setOpenItemId(null)} onRestore={() => onRestore(openItem)} onDelete={() => onPermDelete(openItem)} />
      )}
    </div>
  );
}

function ArchiveDrawer({ item, archiveFolders, onClose, onRestore, onDelete }: { item: ArchiveItem; archiveFolders: ArchiveFolder[]; onClose: () => void; onRestore: () => void; onDelete: () => void }) {
  return (
