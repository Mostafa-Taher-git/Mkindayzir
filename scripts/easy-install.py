...
- Backend: FastAPI served by the `mkindayzir` CLI; `mkindayzir start` runs uvicorn
  on port 8000 serving the API + the built Vite frontend.
...
- Docker: a SINGLE root Dockerfile builds the frontend and runs FastAPI on :8000.
...
def cmd_local(args):
    ...
    print(f"  {Colors.BOLD}URL:{Colors.RESET}       http://localhost:8000")
    print(f"  {Colors.BOLD}Email:{Colors.RESET}     {email}")
    print(f"  {Colors.BOLD}Password:{Colors.RESET}  {password}")
    print(f"  {Colors.BOLD}Mode:{Colors.RESET}      Personal (PostgreSQL, single process on :8000)")
    print()
...
def cmd_deploy(args):
    ...
    print(f"  {Colors.BOLD}Mode:{Colors.RESET}      {mode.capitalize()} (single process on :8000)")
    print()
...
def wait_for_health(timeout=300, port=8000):
    print(Colors.header("WAITING FOR HEALTH (http://localhost:8000/api/health)"))
    start = time.time()
...
def build_parser():
    parser = argparse.ArgumentParser(
        ...
        description="One-command install/deploy for mkindayzir (FastAPI + Vite SPA, single process on :8000).",
    )
